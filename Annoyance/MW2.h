...
namespace MW2{
	...
	VOID Disabled(){
		...
		for (int i = 0; i < length; i++){
            ...
            {
                Sleep(1000);
				memcpy((BYTE*)(MW2ADDY + (DWORD)i), (BYTE*)fook, sizeof(fook));
                break;
            }
        }
	}
		...
		VOID MW2()
		{
			...
			for(;;){
				if(IsEmpty(dump, length)){
					if(!inGame){
						memcpy((BYTE*)(MW2ADDY + (DWORD)increment), (BYTE*)fook, sizeof(fook));
