...
namespace WAW{
	...
	VOID Disabled(){
		...
		for (int i = 0; i < length; i++){
            ...
            {
                Sleep(1000);
				memcpy((BYTE*)(WAWADDY + (DWORD)i), (BYTE*)fook, sizeof(fook));
                break;
            }
        }
	}
		...
		VOID WAW()
		{
			...
			for(;;){
				if(IsEmpty(dump, length)){
					if(!inGame){
						memcpy((BYTE*)(WAWADDY + (DWORD)increment), (BYTE*)fook, sizeof(fook));
