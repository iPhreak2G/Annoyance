...
namespace MW1{
	...
	VOID Disabled(){
		...
		for (int i = 0; i < length; i++){
            ...
            {
                Sleep(1000);
				memcpy((BYTE*)(MW1ADDY + (DWORD)i), (BYTE*)fook, sizeof(fook));
                break;
            }
        }
	}
		...
		VOID MW1()
		{
			...
			for(;;){
				if(IsEmpty(dump, length)){
					if(!inGame){
						memcpy((BYTE*)(MW1ADDY + (DWORD)increment), (BYTE*)fook, sizeof(fook));
